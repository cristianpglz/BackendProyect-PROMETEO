...
export const seedDatabase = async () => {
    try {
        ...
        if (count === 0) {
            console.log("[AUTO-SEED] No se encontraron animes en la base de datos. Insertando animes iniciales...");
            await Anime.insertMany(animesIniciales);
            console.log("[AUTO-SEED] Animes iniciales insertados con éxito.");
        } else {
            console.log(`[AUTO-SEED] Ya existen ${count} animes. Omitiendo la carga inicial.`);
        }
    } catch (error) {
        console.error("[AUTO-SEED ERROR] Falló la ejecución del script de sembrado:", error.message);
    }
};